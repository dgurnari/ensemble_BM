...
def similarity_matrix_from_BM(bm):
    ...
    all_points = sorted(
        set().union(
            *[bm.Graph.nodes[node]["points covered"] for node in bm.Graph.nodes]
        )
    )
...
def ensemble_BM(X, eps_list, num_it, seed=42, verbose=None, **kwargs):
    ...
    verbose: string, default=None
        set to `tqdm` for a tqdm progress bar
    ...
    for eps in tqdm(eps_list, disable=not (verbose == "tqdm")):
        for i in tqdm(range(num_it), disable=not (verbose == "tqdm")):
    ...
    return sim / (num_it * len(eps_list))
    ...
    for n1 in ensemble_cc.keys():
        for n2 in ccT.keys():
            ...
            B.add_edge(
                ...
                n2,
                weight=-1
                * len(ensemble_cc[n1].intersection(ccT[n2]))
                / len(ensemble_cc[n1].union(ccT[n2])),
            )
...
def ensemble_BM_cc(X, eps_list, default_cc, num_it, seed=42, **kwargs):
    """
